<?php
    ...
    function insert_coup_buy2($url,$category){
        ...
        while (!feof($handle)){
            $text = fgets($handle);
            $discount_pattern = '/id="PriceDiscount">([\\d]*)/';
            $price_pattern = '/<span id="DealPrice" style="font-size:36px;">([\\d,]*?)</'; //the pattern to find the price take in calc that the price may be more than 3 digits.
